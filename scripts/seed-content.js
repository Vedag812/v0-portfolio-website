// Seed script: push full default content to Redis via the content API
const BASE = process.argv[2] || "http://localhost:3000";
const TOKEN = process.argv[3] || process.env.ADMIN_TOKEN;
if (!TOKEN) { console.error("Usage: node seed-content.js [base-url] <admin-token>"); process.exit(1); }
...
const defaults = {
    ...
    experiences: [
        ...
        },
        {
            ...
            description: [
                ...
                "Optimised platform performance via code splitting and lazy loading, reducing page load time by 36% (4.2s to 2.7s)",
            ],
            type: "Club Activity",
        },
    ],
    achievements: [
        { title: "Harvard HPAIR Delegate", detail: "Selected among participants from 70+ countries for the Harvard Project for Asian and International Relations conference" },
        { title: "Smart India Hackathon 2024", detail: "Qualified for national-level government hackathon run by India's Ministry of Education, competing among 100,000+ student teams" },
    ],
    certifications: [
    ...
    skills: [
        { title: "Programming Languages", icon: "Code", color: "text-blue-400", skills: [{ name: "Python", level: 92 }, { name: "SQL (PostgreSQL, MySQL)", level: 88 }, { name: "JavaScript", level: 80 }, { name: "C / C++", level: 70 }, { name: "Java", level: 65 }] },
        { title: "Data Analysis & Statistics", icon: "BarChart3", color: "text-emerald-400", skills: [{ name: "Pandas / NumPy", level: 92 }, { name: "EDA & Data Wrangling", level: 90 }, { name: "Hypothesis Testing / A/B Testing", level: 85 }, { name: "Statistical & Quantitative Analysis", level: 85 }, { name: "Data Mining & Modelling", level: 80 }] },
        { title: "Machine Learning & NLP", icon: "Brain", color: "text-red-400", skills: [{ name: "Scikit-learn / XGBoost", level: 88 }, { name: "TensorFlow / Keras", level: 82 }, { name: "NLTK / TF-IDF / TextBlob", level: 85 }, { name: "Feature Engineering", level: 80 }, { name: "Transformers & Sentiment Analysis", level: 75 }] },
        { title: "Visualization & Reporting", icon: "Database", color: "text-amber-400", skills: [{ name: "Power BI", level: 88 }, { name: "Tableau", level: 82 }, { name: "Matplotlib / Seaborn / Plotly", level: 85 }, { name: "Dashboard Design", level: 80 }, { name: "Stakeholder Presentations", level: 85 }] },
        { title: "Cloud & Tools", icon: "Globe", color: "text-purple-400", skills: [{ name: "GCP (Google Cloud)", level: 70 }, { name: "Git / GitHub", level: 90 }, { name: "Jupyter / Google Colab", level: 92 }, { name: "ETL Pipelines / REST APIs", level: 80 }, { name: "Supabase / Gradio", level: 75 }] },
        { title: "Web Development", icon: "Code", color: "text-cyan-400", skills: [{ name: "React.js / Next.js", level: 82 }, { name: "Node.js", level: 72 }, { name: "HTML / CSS / Tailwind", level: 88 }, { name: "TypeScript", level: 75 }] },
    ],
    contact: {
...
async function seed() {
    console.log(`Seeding content to ${BASE}/api/content ...`);
    ...
    // Verify
    const check = await fetch(`${BASE}/api/content`);
    const content = await check.json();
    console.log("Verification — has CGPA:", !!content.about?.cgpa, "experiences:", content.experiences?.length, "skills cats:", content.skills?.length);
}
